...
(function () {
  ...
  supabaseClient.auth.onAuthStateChange(function (event) {
    ...
    // Re-read the profile role whenever Supabase restores or refreshes the
    // authenticated session. Tech Support updates profiles.role, which is the
    // canonical role consumed by this application. This prevents a stale
    // in-memory Staff role from surviving a token/session refresh.
    if (event === "SIGNED_IN" || event === "TOKEN_REFRESHED" || event === "USER_UPDATED") {
      setTimeout(async function () {
        try {
          await refreshData();
          await refreshSessionUser();
          if (PAGE === "app" && currentUser) render();
        } catch (e) {
          console.warn("Role/session refresh failed:", e);
        }
      }, 0);
    }
  });
  ...
  setInterval(async function () {
    if (!session()) return;
    try {
      var before = currentUser && currentUser.role;
      await refreshData();
      await refreshSessionUser();
      if (currentUser && currentUser.role !== before) render();
      else renderChrome();
    } catch (e) {
      console.warn("Background role refresh failed:", e);
    }
  }, 30000);
  init();
})();
