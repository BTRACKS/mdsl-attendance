...
(function () {
  ...
  function messageDate(ts) {
    if (!ts) return "";
    var d = new Date(ts);
    if (isNaN(d.getTime())) return "";
    var today = new Date();
    if (d.toDateString() === today.toDateString()) return messageTime(ts);
    return d.toLocaleDateString("en-GB", { day: "numeric", month: "short" });
  }
  function messageConversationCacheKey() {
    var u = session();
    return "md_message_conversations_v1_" + (u ? u.id : "");
  }
  ...
  function writeMessageConversationCache(conversations) {
    try { sessionStorage.setItem(messageConversationCacheKey(), JSON.stringify(conversations || [])); } catch (e) {}
  }
  ...
  async function fetchConversationList() {
    ...
    var result = await Promise.all((c.data || []).map(async function (conv) {
      ...
      /* Fetch each conversation's latest message and unread receipts concurrently.
         The previous version did these requests one conversation at a time, which
         made a reload unnecessarily slow when several conversations existed. */
      var lastPromise = supabaseClient.from("messages").select("id,sender_id,ciphertext,iv,created_at").eq("conversation_id", conv.id).order("created_at", { ascending: false }).limit(1);
      var unreadPromise = supabaseClient.from("messages").select("id,sender_id").eq("conversation_id", conv.id).neq("sender_id", u.id);
      var pair = await Promise.all([lastPromise, unreadPromise]);
      var last = pair[0], unreadRows = pair[1];
    ...
    result.sort(function(a,b){
      var ad = a.last && a.last.created_at ? new Date(a.last.created_at).getTime() : new Date(a.created_at || 0).getTime();
      var bd = b.last && b.last.created_at ? new Date(b.last.created_at).getTime() : new Date(b.created_at || 0).getTime();
  ...
  async function loadMessagingData() {
    var cached = readMessageConversationCache();
    if (cached && cached.length) {
      messageState.conversations = cached;
      refreshMessageUnread().catch(function(){});
      /* Refresh the list in the background; the cached list keeps the UI instant. */
      fetchConversationList().then(function(fresh) {
        ...
        refreshMessageUnread().catch(function(){});
      }).catch(function(e){ console.warn("Messaging list refresh:", e); });
      return;
    }
  ...
  function bindConversationList() {
    Array.prototype.forEach.call(document.querySelectorAll("[data-conversation]"),function(b){b.addEventListener("click",function(){messageState.activeConversation=b.getAttribute("data-conversation");try{sessionStorage.setItem("md_active_message_conversation_" + session().id, messageState.activeConversation);}catch(e){}renderMessagesKeepState();loadMessagesForConversation(messageState.activeConversation);});});
  }
  ...
  function render() {
    ...
    if (hash === "#/admin" || hash === "#/admin/attendance" || hash === "#/admin/hse") {
      ...
      if (!messageState.activeConversation) {
        try { messageState.activeConversation = sessionStorage.getItem("md_active_message_conversation_" + session().id) || null; } catch (e) {}
      }
      ...
      if (messageState.unlockedPrivateKey) {
        if (cachedConversations && cachedConversations.length) {
          messageState.conversations = cachedConversations;
          renderMessagesKeepState();
        }
        loadMessagingData().then(function(){
          if (location.hash === "#/messages") renderMessagesKeepState();
        }).catch(function(e){ console.warn("Messaging load:", e); });
      }
      return;
    } else {
